...
# cooccurance- build up association between words 
data(stop_words) # Stop words.
real_resume <- readLines("~/Downloads/MarshResumeJan2021_Data_Mining.txt")
real_resume <- str_replace_all(real_resume, pattern = '\"', replacement = "")
...
=======
>>>>>>> 561f9b0f75eff4be8a643139669291bb12fece38

```{r}
similarities_0 <- sim2(tf_idf_1, method = "cosine", norm = "l2") 
get_similar_letters <- function(similarities, reference_letter, n_recommendations = 8){
  sort(similarities[reference_letter, ], decreasing = TRUE)[1:(2 + n_recommendations)]
}
get_similar_letters(similarities_0, 1)
...
index <-names(get_similar_letters(similarities, 1))[-1]
get_substring <- function(string,nchar = 4){
  return(substr(string,nchar+1,nchar(string)))
}
index_desc <- sapply(index,get_substring)
index_true_desc_1 <- as.numeric(index_desc) - 1
desc[412]
```
...
idf_resume <- 1/doc_freq_resume
idf_mat_resume <- rep(idf_resume,nrow(freqs_mat_resume), byrow = TRUE, nrow = nrow(freqs_mat_resume))
tf_idf_resume_1 <- freqs_mat_resume * idf_mat_resume


names(tf_idf_resume_1) <- colnames(tf_idf_resume_1)
names(tf_idf_1) <- colnames(tf_idf_1)
resume_desc_1 <- as.matrix(bind_rows(tf_idf_resume_1,tf_idf_1))
resume_desc_1[which(is.na(resume_desc_1))] <- 0
names(resume_desc_1) <- colnames(resume_desc_1)
similarities_1 <- sim2(resume_desc_1, method = "cosine", norm = "l2") 
colnames(similarities_1) <- c('resume',1:nrow(tf_idf_1))
get_similar_letters <- function(similarities, reference_letter, n_recommendations = 8){
  sort(similarities[reference_letter, ], decreasing = TRUE)[1:(2 + n_recommendations)]
}
get_similar_letters(similarities_1, 1)
