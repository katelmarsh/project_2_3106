...
recommend_resume <- function(file_path = file_path, n_recommendations = n_recommendations){
  ...
  diversity_metric <- rep(0, length(recommended_position))
    for(i in 1:length(recommended_position)){
      if(recommended_position[i] %in% duplicated(recommended_position)){
        diversity_metric[i] <- diversity_metric[i]+1}
      if(cluster_name[i] %in% duplicated(cluster_name)){
        diversity_metric[i] <- diversity_metric[i]+1}
    }   
  
  recommended_result = cbind(recommended_result, diversity_metric)
  recommended_result <- recommended_result[!duplicated(recommended_result[,"recommended_position"]),]
  recommended_result <- recommended_result[!duplicated(recommended_result[,"cluster_name"]),]
  #recommended_result <- as.data.frame(recommended_result)
  #recommended_result[with(recommended_result, order(similarity, cluster_name, recommended_position)),]
...
result3 <- recommend_resume("Retail_Sales_Associate_Resume.txt", n_recommendations = 50)
...
par(new = TRUE)
plot(result1[,"similarity"], col = "blue", ylim=c(0.05,.14), xlim=c(1,10), ylab=" ", pch=16)
par(new = TRUE)
...
```{r}
# good metric is similarity metric normalized 


good_score <- normalize(as.numeric(result["similarity",]))

# diversity metric is a scale of 0-2 with 0 being no overlap with first rec, 1 being 1 overlap, and 2 being two overlaps

diversity_metric <- function(x) {
  metric <- rep(0, n_recommendations)
  for(i in 1:n_recommendations){
    if(result["recommended_position", 1] ==  result["recommended_position", i]){
      metric[i] <- metric[i]+1}
    if(result["cluster_name", 1] ==  result["cluster_name", i]){
      metric[i] <- metric[i]+1}
  } 
  metric[1] <- 0 
  return(metric)
}
diverse_score <- diversity_metric(result)

# OR 
diversity_metric <- rep(0, length(recommended_position))
  for(j in 1:n_recommendations){
    for(i in 1:length(recommended_position)){
      if(recommended_position[j] ==  recommended_position[i]){
        diversity_metric[i] <- diversity_metric[i]+1}
      if(cluster_name[j] ==  cluster_name[i]){
        diversity_metric[i] <- diversity_metric[i]+1}
    } 
    diversity_metric[j] <- diversity_metric[j]-2

result <- rbind(result, good_score)
result <- rbind(result, diverse_score)
